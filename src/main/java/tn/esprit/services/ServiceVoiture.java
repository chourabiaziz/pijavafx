...
public class ServiceVoiture implements IService<Voiture> {
    ...
    public void add(Voiture voiture) {
        //1-req sql INSERT
        //2-executer req
        String qry = "INSERT INTO `voiture`(`id`, `marque`, `modele`, `annee_fabrication`, `numero_serie`, `type_carburant`, `numero_immatriculation`, `kilometrage`, `couleur`, `prix_achat`, `prix_actuel`, `date_achat`, `carte_grise`, `nom_image`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            ...
            pstm.setInt(4, voiture.getAnnee_fabrication());
            pstm.setString(5, voiture.getNumero_serie());
            pstm.setString(6, voiture.getType_carburant());
            pstm.setString(7, voiture.getNumero_immatriculation());
            pstm.setInt(8, voiture.getKilometrage());
            pstm.setString(9, voiture.getCouleur());
            pstm.setDouble(10, voiture.getPrix_achat());
            pstm.setDouble(11, voiture.getPrix_actuel());
            pstm.setDate(12, new Date(2023,02,02));
            pstm.setString(13, voiture.getCarte_grise());
            pstm.setString(14, voiture.getNom_image());
    ...
    public ArrayList<Voiture> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                v.setAnnee_fabrication(rs.getInt("annee_fabrication"));
    ...
    public void update(Voiture voiture) {
        String qry = "UPDATE `voiture` SET `marque`=?, `modele`=?, `annee_fabrication`=?, `numero_serie`=?, `type_carburant`=?, `numero_immatriculation`=?, `kilometrage`=?, `couleur`=?, `prix_achat`=?, `prix_actuel`=?, `date_achat`=?, `carte_grise`=?, `nom_image`=? WHERE `id`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
            pstm.setString(1, voiture.getMarque());
            pstm.setString(2, voiture.getModele());
            pstm.setInt(3, voiture.getAnnee_fabrication());
            ...
            pstm.setDate(11, new Date(voiture.getDate_achat().getTime())); // Convertir la date en java.sql.Date
            pstm.setString(12, voiture.getCarte_grise());
            pstm.setString(13, voiture.getNom_image());
            pstm.setInt(14, voiture.getId());
