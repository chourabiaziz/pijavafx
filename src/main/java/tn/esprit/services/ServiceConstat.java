...
import tn.esprit.interfaces.IServiceMariem;
...
public class ServiceConstat implements IServiceMariem<Constat> {
    ...
    public Map<String, Map<String, Integer>> getConstatsPerUser() {
        Map<String, Map<String, Integer>> stats = new HashMap<>();
        String query = "SELECT A_preneur_nom, A_preneur_prenom, COUNT(*) AS num_constats " +
                "FROM constat " +
                "GROUP BY A_preneur_nom, A_preneur_prenom";
        try (PreparedStatement pstmt = cnx.prepareStatement(query)) {
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                String preneurNom = rs.getString("A_preneur_nom");
                String preneurPrenom = rs.getString("A_preneur_prenom");
                int numConstats = rs.getInt("num_constats");

                Map<String, Integer> preneurStats = stats.getOrDefault(preneurNom, new HashMap<>());
                preneurStats.put(preneurPrenom, numConstats);
                stats.put(preneurNom, preneurStats);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public PieChart getPieChart() {
        // Récupérez les statistiques des constats par utilisateur
        Map<String, Map<String, Integer>> userConstats = getConstatsPerUser();

        // Créez un nouveau PieChart
        PieChart pieChart = new PieChart();

        // Ajoutez les données au PieChart
        for (Map.Entry<String, Map<String, Integer>> entry : userConstats.entrySet()) {
            for (Map.Entry<String, Integer> innerEntry : entry.getValue().entrySet()) {
                PieChart.Data data = new PieChart.Data(entry.getKey() + " " + innerEntry.getKey(), innerEntry.getValue());
                pieChart.getData().add(data);
            }
        }

        // Retournez le PieChart
        return pieChart;
    }
